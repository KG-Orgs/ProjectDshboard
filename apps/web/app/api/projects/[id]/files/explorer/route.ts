...
function buildQuery(request: NextRequest): string {
  ...
  for (const key of ['folderPath', 'projectRootFolderName']) {
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${getBackendBaseUrl()}/api/projects/${encodeURIComponent(context.params.id)}/files/explorer${query}`,
      {
        method: 'GET',
    ...
    console.error('Get project explorer folder error:', error);
