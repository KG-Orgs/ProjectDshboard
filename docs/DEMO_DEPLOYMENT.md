...
| Chat works but PDFs will not open | Render has no local OneDrive disk. Deploy the Graph file-content fallback. A **project admin** must **Connect OneDrive** and bind the project folder once; after that, members can open PDFs without connecting their own OneDrive. Use **MLJ-017 Package 6 - General**, not the TEST CLONE. |
