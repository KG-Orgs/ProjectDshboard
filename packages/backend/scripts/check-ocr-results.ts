...
async function main() {
  ...
  const targets = [
    "%GEN-021R00%Diego%",
    "%NOR-010R00%CCTV%",
    "%GEN-014R00%Monthly%Quality%",
    "%GEN-027R00%Crossroads%",
    "%BUR-001R00%Staircase%",
  ];
  ...
  for (const pat of targets) {
    ...
    if (withChunks.length > 0) {
      console.log(`✅ [${pat}] chunks=${withChunks[0].cnt} | ${withChunks[0].id} | ${withChunks[0].fn.slice(0, 70)}`);
    } else {
      console.log(`❌ [${pat}] still 0 chunks (${rows.length} files found)`);
    }
  }
}
